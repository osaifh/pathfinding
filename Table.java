...
public class Table {
    ...
    public void updateEdges(int xx, int xy, int yx, int yy){
        for (int i = xx; i < yx; ++i){
            for (int j = xy; j < yy; ++j){
                Edge aux = isEdge(i,j);
                if (aux==Edge.NW) tab[i][j].setID(6);
                else if (aux==Edge.NE) tab[i][j].setID(7);
                else if (aux==Edge.SW) tab[i][j].setID(8);
                else if (aux==Edge.SE) tab[i][j].setID(9);
            }
        }
    }
    ...
    public void dark(int time){
        ...
        if (time >=0 && time<100){
            ...
            light_level=100-((time-100)*5);
        } else if (time>=220 && time <= 240){
            light_level=(time-220)*5;
        }
        for (int i = 0; i < TABLE_SIZE; ++i){
            for (int j = 0; j < TABLE_SIZE; ++j){
